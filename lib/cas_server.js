...
/** @param {string} ticket 
 ...
 function save_sessionID(sessionStore, ticket, sessionID) {    
    const expires = Date.now() + conf.uid_validity_seconds * 1000
    sessionStore.set(ticket, { sessionID, cookie: { expires } }) // NB: mongodb sessionStore will use cookie.expires to know the TTL to use
}
...
function expire_session_uid_and_otp_from_ticket(sessionStore, ticket) {
    sessionStore.get(ticket, (err, ticket_info) => {
        if (err) {
            ...
            const sessionID = ticket_info.sessionID
            if (sessionID) {
    ...
    new Promise((resolve, reject) => {
        sessionStore.get(ticket, (err, info) => {
            ...
            if (!info) {
                ...
                console.log("keeping ticket for sessionID", ticket, info.sessionID)
                save_sessionID(sessionStore, ticket, info.sessionID) 
            } else {
