...
import range from "ip-range-check";

import * as cond_no_otp from "./cond_no_otp.js";
import * as h from "./helpers.js"
import conf from "../conf.js";
import * as sessionsPage from './sessionsPage.js';

/** @typedef {{ uid: string, raw_response: string }} v2_response */
/** @typedef {{ uid: string, service: string, v2_response?: v2_response, date: number, validated_otp: boolean, sessionID: string, addServiceHashtoPath: boolean }} ticket_info */

/** @typedef {{}} empty_session */
/** @typedef {{ uid: string, validated_uid: Date, ticket_for_SLO: string, ip?: string, userAgent?: string, lastUseDate: Date, ticket_to_v2_response: Object.<string, v2_response> }} session_but_not_validated (ticket consumed) */
/** @typedef {{ uid: string, validated_uid: Date, ticket_for_SLO: string, ip?: string, userAgent?: string, lastUseDate: Date, attrs?: Object.<string, string[]>, validated_FranceConnect_AND_password?: true, validated_otp: Date, validated_method: string, long_term_otp: boolean }} session_validated */


export const hooks = {
    /** @type ((req, ticket_info: ticket_info) => void)[] */
    before_save_allowed_ticket: [],
    /** @type ((req, ticket_info: ticket_info, ticket: string, service: string) => Promise<void>)[] */
    after_validate_ticket: [],
}
...
const remove_query_params = (url) => (
    url.replace(/[?].*/, '')
)

/**
 * @param {{ service: string, renew?: string, ask?: string, ticket?: string, pgtUrl?: string }} query 
 * @param {(boolean | 'with_query_params')=} addServiceHashtoPath
 */
export function ourUrl(query, addServiceHashtoPath) {
    // ensure CAS tracks all the tickets
    // By default, Apereo CAS only keeps the latest ticket for each "normalized" service
    // Normalization is : removing the query string and everything after a semi-colon. Cf normalizePath in https://github.com/apereo/cas/blob/6.6.x/core/cas-server-core-tickets-api/src/main/java/org/apereo/cas/ticket/DefaultServiceTicketSessionTrackingPolicy.java
    // Solution: add a hash of the service in the URL passed to "cas server validating password". Eg: /otp/login-c9d23612e339232cef13c6dd33571277?service=...
    return conf.our_base_url + '/login' + (addServiceHashtoPath && query.service ? '-' + (
        h.md5(addServiceHashtoPath === 'with_query_params' ? query.service : remove_query_params(query.service))
 ...
 * @param {{}} [opts={}] { method:'POST' } 
 */
export function fetchFromCas(path, params, opts) {
    return fetch(conf.cas_server_validating_password_base_url_internal + path + "?" + querystring.stringify(params), opts);
}


/**
 * no need to wait for a very slow answer since ticket will be invalid anyway
 * @param {String} path
 * @param {import('querystring').ParsedUrlQuery} params
 * @param {{}} [opts={}] { method:'POST' } 
 */
function fetchFromCasWithTimeout(path, params, opts) {
    return fetchFromCas(path, params, {
        signal: AbortSignal.timeout(conf.ticket_validity_seconds * 1000),
        ...opts,
    });
}
...
async function casv1_validate_ticket(service, ticket, addServiceHashtoPath) {
    ...
    const response = await fetchFromCasWithTimeout('/validate', params)
    if (response.ok) {
        const body = await response.text();
...
async function casv2_validate_ticket(service, ticket, addServiceHashtoPath, pgtUrl) {
    ...
    const response = await fetchFromCasWithTimeout('/serviceValidate', params)
    if (response.ok) {
        const body = await response.text();
...
function set_validated_otp(req, method) {
    ...
    req.session.validated_method = method;
...
const shouldAddServiceHashtoPath = (req) => {
    const m = req.path.match(/login-(\w+)/)
    if (m) {
        return h.md5(req.query.service) === m[1] ? 'with_query_params' : true
    } else {
        return false
    }
}
...
function onLoginSuccess(req, res, ticket, consumed_ticket) {
    ...
    if (service == sessionsPage.URL) {
        return sessionsPage.display(req, res)
    }
    ...
    let ticket_info = { 
        ...
        v2_response,
        date: Date.now(),
        validated_otp: Boolean(req.session.validated_otp),
        sessionID: req.sessionID,
        addServiceHashtoPath: shouldAddServiceHashtoPath(req),
    }
    for (const hook of hooks.before_save_allowed_ticket) {
        hook(req, ticket_info)
    }
...
async function validate_ticket(req, cas_validate_ticket) {
    ...
    for (const hook of hooks.after_validate_ticket) {
        await hook(req, ticket_info, ticket, service)
    }  
...
const is_req_allowed_ = (req, conds, cond_name) => (
    conds.if_IP_in ? (
        conds.if_IP_in && range(req.ip, conds.if_IP_in)
...
async function isOtpValidForService(req) {
    if (!get_valid_uid(req.session)) {
        return false;
    }

    // service without OTP
    if (await cond_no_otp.no_otp(req)) {
        return true;
    }

    // check OTP is still valid for service
    return !is_validated_otp_too_old(req.session.validated_otp, req.query.service);
}
...
export function routing() {
    ...
    router.get('/login*', handle_error(async function(req, res) {
        ...
        req.session.lastUseDate = new Date();
    ...
    router.post('/login*', handle_error(async function(req, res) {
        if (req.body.logoutRequest) {
            ...
            res.send('')
        } else if (req.body.rememberMe && req.session.validated_otp && await isOtpValidForService(req)) {
            ...
            const cookie_for_context = req.header('cookie')?.match(/(connect.sid=[^; ]*)/)?.[1]
            const { uid_hint } = req.body
            if (!cookie_for_context) {
                h.my_log(req, "missing session cookie", { uid_hint })
                res.send(`<p>Votre navigateur n'accepte pas correctement les cookies. Veuillez essayer un autre navigateur ?</p>
                            <p><button onclick="location.href = location.href">Réessayer</button></p>`)
                return
            }
            h.my_log(req, "expired session?", { cookie_for_context, uid_hint })
    ...
    router.get('/proxyValidate', handle_error(async function (req, res) {
        if (req.query.ticket?.match(/^PT-/)) {
            ...
            res.header('Content-Type', 'application/xml; charset=UTF-8')
            res.send(await (await fetchFromCasWithTimeout('/proxyValidate', req.query)).text())
        } else {
    ...
    router.get('/proxy', handle_error(async function (req, res) {
        // proxy unchanged request to cas_server_validating_password
        res.header('Content-Type', 'application/xml; charset=UTF-8')
        res.send(await (await fetchFromCasWithTimeout('/proxy', req.query)).text())
    }))

    router.get('/logout', handle_error(async function (req, res) {
        if ("remove_session" in req.query) {
            h.my_log(req, "logout")
            req?.session?.destroy(_ => {})
        }
        if ("noservice" in req.query) {
            res.send("")
        } else {
    ...
    router.get('/session', handle_error(async function (req, res) {
        res.render('session.ejs', { session: h.pick(req.session, "validated_otp") })
    }))

    router.get(`/${sessionsPage.path}`, handle_error(async function(req, res) {
        return res.redirect(ourUrl({ service: sessionsPage.URL }));
    }))

    router.delete(`/${sessionsPage.path}`, handle_error(async function(req, res) {
        req.query.service = sessionsPage.URL;
        if (await isOtpValidForService(req)) {
            await sessionsPage.logout(req.session.uid, req.mongo_collection, req.query);
            return res.end();
        } else {
 ...
 */
export function lookupGeolocation(ip) {
    return fetchApi(`protected/geolocation?ip=${ip}`)
        .then(res => res.json())
        .catch(err => {})
}
