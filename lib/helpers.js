...
/** @param {number} date */
export const seconds_since = (date) => (Date.now() - date) / 1000
...
export function sha256(data) {
    return raw_hash('SHA256', data)
}

/** @param {String} uid */
export function get_hash(api_users_secret, uid) {
    const d = new Date();
    const present_salt = d.getUTCDate()+d.getUTCHours().toString();
    //console.log("present-salt for", uid, ":", present_salt);
    return sha256(md5(api_users_secret) + uid + present_salt);
}

/** @type {<T extends object, K extends keyof T>(object: T, key: K) => T[K]} */
export function getAndDelete(o, key) {
    const val = o[key]
    delete o[key]
    // @ts-ignore
    return val
...
/**
 * @param { (?string | number | Date)[] } dates
 * @returns {number[]}
 */
function datesToTimestamps(dates) {
    return dates.map(dateToTimestamp).filter(Boolean);
}
...
function timestampToDate(timestamp) {
    return ((timestamp && Number.isFinite(timestamp)) || null) && new Date(timestamp);
}
 ...
 * @returns {?Date}
 */
export function olderDate(dates) {
    const timestamps = datesToTimestamps(dates)
    return timestampToDate(Math.min(...timestamps));
}
 ...
 * @returns {?Date}
 */
export function newerDate(dates) {
    const timestamps = datesToTimestamps(dates)
    return timestampToDate(Math.max(...timestamps));
}
...
/** @typedef {{ regexp?: string | RegExp, domains_and_hostnames?: string[] }} service_tests */

/** @param {service_tests} service_tests
  * @param {string} service */
export const is_service_matching = (service_tests, service) => (
...
export async function esupUserApps_canAccess(conf_esupUserApps, uid, app) {
 ...
 */
export function my_log(req_for_context, msg, details) {
 ...
 * @param {Object.<string, any> =} details 
 */
export function my_log_if_ticket(req, msg, details) {
