...
/** @param {string} ticket */
async function expire_session_uid_and_otp_from_ticket(sessionStore, mongo_collection, ticket) {
    try {
        ...
        if (req.body.logoutRequest) {
            if (require("ip-range-check")(req.ip, conf.allow_back_channel_single_logout.if_IP_in)) {
                if (is_service_allowed(req.query.service)) { // sanity check
                    ...
                    expire_session_uid_and_otp_from_ticket(req.sessionStore, req.mongo_collection, ticket) // (done async)
