...
async function login_page(res, uid, ticket, error, params) {
    ...
    const server_infos = methods.esupnfc.server_infos;
    if(server_infos && !server_infos.qrcode) {
        server_infos.qrcode = await qrcode.toString(JSON.stringify(server_infos), {type: "svg"});
    }
