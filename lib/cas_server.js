...
/** @typedef {{ uid: string, validated_uid: Date, ticket_for_SLO: string, validated_FranceConnect_AND_password?: true, validated_otp: Date, long_term_otp: boolean }} session_validated */
...
async function change_logged_user(sessionStore, sessionID, session, new_uid) {
    const other_user_sessionID = (uid) => sessionID + "-" + uid

    if (session.validated_otp) {
        console.log("saving OTP for previous user", session.uid)
        sessionStore.set(other_user_sessionID(session.uid), { ...session })
    }        

    let prev_session = await sessionStore_get(sessionStore, other_user_sessionID(new_uid)) ?? {}
    if (prev_session.validated_otp) {
        console.log("got saved validated_OTP for user", new_uid)
    }        
    // get info from previous saved session, or DROP current infos!
    Object.assign(session, h.pick(prev_session, 'validated_otp', 'validated_FranceConnect_AND_password', 'long_term_otp'))
...
async function get_logged_user_in_session(req, session, service, ticket, addServiceHashtoPath) {
    ...
    if (session.uid && session.uid !== v2_response.uid) {
        ...
        await change_logged_user(req.sessionStore, req.sessionID, session, v2_response.uid)
    }
...
/** @param {string} uid
  * @param {string} ticket
  * @param {string} error */
function login_page(res, uid, ticket, error, params) {
        ...
        if (!get_valid_uid(req.session)) {
            if (req.query.gateway) {
                ...
                try {
                    ...
                    await get_logged_user_in_session(req, req.session, service, ticket, shouldAddServiceHashtoPath(req));
                } catch (err) {
                    console.error(err)
