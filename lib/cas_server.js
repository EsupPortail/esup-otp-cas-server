...
/** 
 * @param {'allow_back_channel_single_logout'} cond_name
 */
...
function routing() {
    ...
    router.get('/login*', handle_error(async function(req, res) {
        ...
        try {
            ...
            let no_need_otp = Boolean(req.session.validated_otp) || await cond_no_otp.no_otp(req)
    ...
    router.post('/login*', handle_error(function(req, res) {
        if (req.body.logoutRequest) {
            ...
            verifyOtp(uid, req.body.token).then(method => {
    ...
    router.get('/proxyValidate', handle_error(async function (req, res) {
        if (req.query.ticket?.match(/^PT-/)) {
            ...
            res.send(await (await fetch(url, { compress: false })).text())
        } else {
