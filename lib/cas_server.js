...
/** @typedef {{ uid: string, validated_uid: Date, ticket_for_SLO: string, validated_otp: Date, long_term_otp: boolean }} session_validated */
...
function routing() {
    ...
    router.post('/login*', handle_error(function(req, res) {
        if (req.body.logoutRequest) {
            ...
            if (!req.session.uid) console.error(req.ip, req.method, req.url, "weird expired session?", req.sessionID, JSON.stringify(req.session))
